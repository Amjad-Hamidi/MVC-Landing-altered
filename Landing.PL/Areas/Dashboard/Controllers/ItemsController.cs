...
using Landing.PL.Data.Migrations;
...
namespace Landing.PL.Areas.Dashboard.Controllers
{
    ...
    public class ItemsController : Controller
    {
        ...
        {
            var portfoilos = context.Portfoilos.ToList();
            var vm = new ItemFormVM
            {
                Portfoilos = new Microsoft.AspNetCore.Mvc.Rendering.SelectList(portfoilos, "Id", "Name")
            };

            /* OR : when i use IEnumerable
            var portfoilos = context.Portfoilos.ToList();
            var vm = new ItemFormVM
            {
                Portfoilos = mapper.Map<IEnumerable<PortfoilosVM>>(portfoilos)
            };
            */
            return View(vm);
        }
        ...
        {   // Mapping
            ...
            var item = mapper.Map<Item>(vm); // <Destination> (Source)

            context.Add(item); // OR :  context.Portfoilo.Add(item);
			context.SaveChanges();
